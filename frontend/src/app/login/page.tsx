...
// Module-level — initialize() must only run once per page load
let _gsiReady = false;

// ─────────────────────────────────────────────────────────────────────────────
// GoogleSignInButton — uses renderButton() NOT prompt()
//
// renderButton() renders Google's own <button> element inside the div.
// This is the ONLY method that:
//   ✅ Never triggers FedCM (no NetworkError, no AbortError)
//   ✅ Never calls initialize() more than once
//   ✅ Works on localhost without HTTPS
//   ✅ Survives user cancelling a previous popup
//
// prompt() triggers Chrome's FedCM silently even with use_fedcm_for_prompt:false
// in newer Chrome — that's what was causing all the console spam.
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ln-left { flex: 0 0 52%; background: #0a1628; display: flex; flex-direction: column; position: relative; overflow: hidden; }
        ...
        .ln-left::after  { content: ''; position: absolute; bottom: -120px; right: -120px; width: 480px; height: 480px; border-radius: 50%; background: radial-gradient(circle, rgba(37,99,235,0.09) 0%, transparent 65%); pointer-events: none; }
        .ln-left-inner { position: relative; z-index: 1; display: flex; flex-direction: column; height: 100%; padding: 52px 56px; }
        .ln-logo { display: flex; align-items: center; gap: 13px; margin-bottom: 72px; }
        ...
        .ln-hero-h { font-size: clamp(1.9rem, 2.8vw, 2.55rem); font-weight: 800; color: #ffffff; letter-spacing: -0.04em; line-height: 1.12; margin-bottom: 16px; }
        .ln-hero-sub { font-size: 15px; color: rgba(255,255,255,0.55); line-height: 1.65; font-weight: 400; max-width: 380px; margin-bottom: 40px; }
      ...
      <div className="ln-root">
        ...
        <div className="ln-right">
          <div className="ln-form-wrap">
            ...
            <GoogleSignInButton onCredential={handleGoogleCredential} loading={loading} text="signin_with"/>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" className="ln-btn" disabled={loading}>
                {loading ? <><div className="ln-spin"/> Signing in…</> : <>Sign in <ArrowRight size={15}/></>}
              </button>
            </form>
